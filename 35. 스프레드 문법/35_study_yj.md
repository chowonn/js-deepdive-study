...
스프레드 문법을 사용할 수 잇는 대상은 for...of 문으로 순회할 수 있는 이터러블에 한정됨
...
- 배열을 펼쳐서 요소들을 개별적인 값의 목록으로 만든 후, Math.max 메서드의 인수로 전달
...
- concat 메서드를 사용해 2개의 배열을 1개의 배열에 결합
...
- 스프레드 문법을 사용하여 별도의 메서드를 사용하지 않고 결합 가능
...
- splice 메서드를 사용해 어떤 배열의 중간에 다른 배열의 요소들을 추가하거나 제거
...
- 이터러블을 배열로 변환하려면 Function.prototype.apply 또는 Function.prototype.call 메서드를 사용하여 slice 메서드를 호출해야 함
...
- 객체 리터럴의 프로퍼티 목록에서도 스프레드 문법을 사용할 수 있음
